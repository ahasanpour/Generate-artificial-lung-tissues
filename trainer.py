...
class Trainer(object):
    ...
    def train(self):
        ...
        for s in xrange(max_steps):
            ...
            if s % 100 == 0:
                losses.append({step,accuracy,d_loss, g_loss, s_loss})
            ...
            if s % 10 == 0:
                ...
                if s%1000 ==0:
                    self.measure.append([accuracy, accuracy_test, d_loss, g_loss, s_loss])
                    if s % 9000==0:
                        scipy.io.savemat(os.path.join(self.train_dir, 'measures_' + str(s) + '.mat'), mdict={'measure': self.measure})
            ...
            if s % output_save_step == 0:
                ...
                if self.config.dump_result:
                    ...
                    f['image'] = g_img
                    scipy.io.savemat(os.path.join(self.train_dir, 'g_img_'+str(s)+'.mat'), mdict={'g_img': g_img})
                    f.close()
    ...
    def run_single_step(self, batch, step=None, is_train=True):
        ...
        if (step==0)or (step%(self.config.update_rate+1)>0):
            fetch.append(self.d_optimizer)
        else:
            fetch.append(self.g_optimizer)
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='LUNG', choices=['MNIST', 'SVHN', 'CIFAR10','LUNG'])
    ...
    if config.dataset == 'MNIST':
        ...
        import datasets.lung as dataset
    else:
